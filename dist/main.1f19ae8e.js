...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"../../AppData/Roaming/nvm/v8.11.2/node_modules/parcel/node_modules/process/browser.js":[function(require,module,exports) {
...
},{"process":"../../AppData/Roaming/nvm/v8.11.2/node_modules/parcel/node_modules/process/browser.js"}],"data.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.data = exports.clothingTypes = exports.fabrics = exports.cities = void 0;
var cities = ["niš", "beograd", "subotica", "novi sad"];
exports.cities = cities;
var fabrics = ["pamuk", "poliester", "elastin", "sintetika", "vuna", "svila"];
exports.fabrics = fabrics;
var clothingTypes = ["bluza", "majica", "helanke", "pantalone", "haljina", "džemper", "košulja"];
exports.clothingTypes = clothingTypes;
var data = [{
...
(function ($) {
  ...
  me.templateFactory = function (url, name, distributor, price) {
    return "<div class=\"product p-2 bg-white rounded d-flex flex-column flex-wrap align-items-center\">\n        <div class=\"image-wrapper d-flex justify-content-center pb-2\">\n          <img src=\"".concat(url, "\"\n                alt=\"").concat(name, "\">\n        </div>\n        <p>").concat(name, "</p>\n        <p>").concat(distributor, "</p>\n        <p class=\"h4\">Cena: <span>").concat(price, "</span></p>\n      </div>");
  };

  me.renderProducts = function (data) {
    data.forEach(function (item) {
      // console.log('rendering product');
      $(me.proizvodiID).append(me.templateFactory(item.imageURL, item.name, item.distributor, item.price));
    });
  };

  me.setListeners = function () {
    $(me.submitButtonID).on('click', function (event) {
      var form = $(me.formID);
      form.submit(me.handleSubmit(form));
    });
  };

  me.handleSubmit = function (form) {
    // console.log('form submitted');
    var isError = false;
    var resultList = _data.data; // clean the data so far

    $(me.proizvodiID).empty();
    var vrstaTkanine = form.find(me.vrstaTkanineID).val().toLowerCase();

    if (_data.fabrics.includes(vrstaTkanine)) {
      // console.log(`there is vrstaTkanine: ${vrstaTkanine}`);
      resultList = resultList.filter(function (item) {
        return item.fabric === vrstaTkanine;
      });
    } else {
      isError = true; // console.log('error with vrstaTkanine');

      me.showTimedError("Ne postoji takva vrsta tkanine u našoj bazi, pokušajte sa nekom drugom.");
      return false;
    }

    var sadržajTkanine = +form.find(me.sadrzajTkanineID).val().toLowerCase();

    if (!isError) {
      // console.log(`there is sadržajTkanine: ${sadržajTkanine}`);
      resultList = resultList.filter(function (item) {
        return item.fabricPercent <= sadržajTkanine;
      });
    } else {
      isError = true; // console.log('error with sadržajTkanine');

      return false;
    }

    var vrstaOdece = form.find(me.vrstaOdeceID).val().toLowerCase();

    if (_data.clothingTypes.includes(vrstaOdece) && !isError) {
      // console.log(`there is vrstaOdece: ${vrstaOdece}`);
      resultList = resultList.filter(function (item) {
        return item.clothingType === vrstaOdece;
      });
    } else if (vrstaOdece === '') {} else {
      isError = true; // console.log('error with vrstaOdece');

      me.showTimedError("Ne postoji takva vrsta odeće u našoj bazi, pokušajte sa nekom drugom.");
      return false;
    }

    var lokacija = form.find(me.lokacijaID).val().toLowerCase();

    if (_data.cities.includes(lokacija) && !isError) {
      // console.log(`there is lokacija: ${lokacija}`);
      resultList = resultList.filter(function (item) {
        return item.city === lokacija;
      });
    } else {
      isError = true; // console.log('error with lokacija');
    ...
    if (isError) {
      me.showTimedError("Došlo je do greške, pokušajte ponovo."); // me.clearForm(form);
    }

    if (resultList.length === 0) {
      me.showTimedError("Ne postoji prašak u našoj bazi koji odgovara datim parametrima. Pokušajte ponovo."); // me.clearForm(form);
    }
...
},{"jquery":"node_modules/jquery/dist/jquery.js","./data":"data.js"}],"../../AppData/Roaming/nvm/v8.11.2/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
...
},{}]},{},["../../AppData/Roaming/nvm/v8.11.2/node_modules/parcel/src/builtins/hmr-runtime.js","main.js"], null)
